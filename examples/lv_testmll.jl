...
@model function PFmodel(c, Y, t, sig_y, nspecies, nreacts, HazardFun,
						...
						N_stiff, H_mult, epsilon, delta, track=false)

	# Take the first observation as a starting point.
    X_curr ~ MvNormal(Y[:,1], sig_y)

	# Track the latent states if needed.
	if track
...
exact_param = PFmodel(c, Y, t, 1.0, nspecies, nreacts, HazardFun,
					  StoichMatrix, ReactPairs, RelEpsilon, 10,
					  100, 5.0, 0.05, 0.05, track=false)

# Arbitrary parameters.
c_arb = [0.5, 0.0025, 0.3]

# PF model with exact parameters.
arb_param = PFmodel(c_arb, Y, t, 1.0, nspecies, nreacts, HazardFun,
					StoichMatrix, ReactPairs, RelEpsilon, 10,
					100, 5.0, 0.05, 0.05, track=false)
